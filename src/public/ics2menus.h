...
// 006: захват adjustable-строки — SetAdjustCapture + MenuLabel::Capture. В capture-меню E на
//      adjustable-строке включает/выключает «захват»: пока строка захвачена, W/S не двигают
//      курсор (настройка не слетает с полёта на WASD), A/D крутят значение, стрелки ◄ ►
//      рисуются ТОЛЬКО у захваченной строки и вся строка красится captureColor. Без захвата
//      adjustable-строка выглядит как обычный пункт. Дефолт off — меню без SetAdjustCapture
//      ведут себя ровно как в 004/005 (стрелки всегда, A/D сразу).
//
// Версия в СТРОКЕ интерфейса — единственная защита ABI: новые методы дописаны в ХВОСТ vtable,
// поэтому старый cs2menus.so отдаёт вызывающему vtable без них. Запрос по строке "…006" у
// старой сборки просто не найдёт интерфейс (потребитель обязан откатиться на "…005"/"…004" и
// не звать новые методы), тогда как совпадение строк гарантирует полный vtable.
#define CS2MENUS_INTERFACE "ICS2Menus006"
// Предыдущие ревизии — для фолбэка потребителя, если рядом оказалась старая сборка cs2menus.
// Полученный по ним указатель ЗАПРЕЩЕНО использовать для методов более новых секций.
#define CS2MENUS_INTERFACE_005 "ICS2Menus005"
...
enum class MenuLabel : int
{
	...
	Capture,  // 006: HTML footer hint for the E capture toggle on adjustable rows (capture menus)
...
public:
	...
	//   Ориентир — kHtmlWrapChars в menu_manager.cpp, и он В КОЛОНКАХ: CJK-глиф считается за
	//   два. Порог эмпирический и калибруется вживую; его рекалибровка вниз молча укорачивает
	//   и эту строку — перепроверяйте потребителей вместе с ним.
	// - Считается в бюджете высоты панели наравне с футером: окно пунктов ужимается. Если по
	//   сумме «заголовок + футер + статус» на пункты не осталось ни строки, статус НЕ рисуется
	//   вовсе — не показать честнее, чем обрезать. ОГОВОРКА: гарантия держится по этой сумме,
	//   а не по фактической высоте — переносящийся пункт всё ещё может выпихнуть панель за
	//   бюджет, и срежется тогда именно статус, как самая нижняя строка.
	// - Разметку НЕ принимает: текст эскейпится, цвет задаётся чат-байтами 0x01..0x10, кегль —
	//   тот же класс, что у пунктов меню.
	// - Звать можно хоть каждый тик: лишний вызов с тем же текстом бесплатен (сравнение
	//   строки), а перерисовку движок откладывает до своего Tick — то есть чаще кадра она не
	//   случится, сколько бы раз метод ни позвали. По умолчанию смена текста перерисовывает
	//   панель на ближайшем кадре (плавные показания, ценой панели в тик); сервер вправе
	//   загрубить это ключом HtmlStatusInterval в core.cfg ради трафика.
	...
	// --- 006: захват adjustable-строки (E фиксирует строку, W/S не сбивают настройку) ---

	// Режим захвата для ЭТОГО меню (дефолт off — поведение 004/005 без изменений).
	// Во включённом режиме:
	//  * adjustable-строка без захвата рендерится как обычный пункт (без стрелок ◄ ►), и A/D на
	//    ней НЕ действуют — сначала захват;
	//  * E (клавиша Select) на adjustable-строке переключает захват вместо onSelect (onSelect
	//    этой строки в capture-меню не зовётся никогда — не вешайте на неё действий);
	//  * пока строка захвачена: W/S заморожены (курсор не движется, страницы не листаются),
	//    A/D зовут adjust-колбэк как раньше, повторная E отпускает; Back/Exit работают штатно;
	//  * захваченная строка рисуется целиком цветом захвата (HtmlCaptureColor в core.cfg,
	//    деф. янтарный #f5c211 против розовой подсветки курсора) и в обрамлении стрелок ◄ ►;
	//  * захват снимается сам при закрытии/уничтожении меню, DisplayMenu другого меню,
	//    переходе в submenu, при удалении строки и очистке пунктов.
	// Только HTML-меню; для chat-меню флаг безвреден и ни на что не влияет.
	virtual void SetAdjustCapture(MenuHandle menu, bool enabled) = 0;
};
