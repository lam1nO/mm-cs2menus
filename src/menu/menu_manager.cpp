...
static int VisibleWidth(const std::string &s)
{
	...
	for (size_t i = 0; i < s.size();)
	{
		...
		// CJK-глиф рисуется в ДВЕ колонки. Считать его за один символ — занижать ширину, а
		// заниженная ширина футера возвращает его обрезку снизу (её чинили в dd37dfd): у
		// китайского футера 31 кодовая точка, но ~39 колонок. Все двухколоночные диапазоны
		// лежат в U+0800..U+FFFF, то есть ровно в трёхбайтовой форме UTF-8 — остальные длины
		// декодировать незачем; усечённую последовательность отдаём общей ветке ниже.
		if ((c & 0xF0) == 0xE0 && i + 2 < s.size())
		{
			const unsigned int cp =
				((c & 0x0Fu) << 12) | ((static_cast<unsigned char>(s[i + 1]) & 0x3Fu) << 6) | (static_cast<unsigned char>(s[i + 2]) & 0x3Fu);
			const bool wide = (cp >= 0x1100 && cp <= 0x115F) || (cp >= 0x2E80 && cp <= 0xA4CF) || (cp >= 0xAC00 && cp <= 0xD7A3)
...
void MenuManager::Tick(float curtime)
{
	...
	for (int i = 0; i <= MAXPLAYERS; i++)
	{
		...
		if (curtime < pm.lastHtmlSend)
		{
			...
			// И дедуп: без сброса свежая разметка совпала бы с домапной, отправка бы не
			// прошла, и панель осталась бы пустой до keep-alive — то есть ровно то замирание,
			// которое эта ветка и лечит, только на пару секунд.
			pm.lastHtml.clear();
